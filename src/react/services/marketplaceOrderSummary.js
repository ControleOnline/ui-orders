/*
 * Regra de negocio: o summary de marketplace deve usar somente campos canonicos
 * do payload. `order_index` e `code` sao identidade; `pickup_code`,
 * `handover_code`, `displayId` e ids tecnicos nao podem virar order code.
 */
...
const buildIfoodOrderSummary = order => {
  ...
  const orderIndex = normalizeText(
    ifoodOrder?.order_index ||
      ifoodOrder?.orderIndex ||
      data?.order_info?.order_index ||
      data?.order_info?.orderIndex ||
      data?.order_index ||
      data?.orderIndex ||
      '',
  )
