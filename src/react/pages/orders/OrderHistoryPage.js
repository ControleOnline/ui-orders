...
const buildOrderHistoryFilterPreferenceScope = companyId => ({
  companyKey: companyId == null || companyId === '' ? '' : String(companyId),
...
const resolveInitialHistoryFilters = companyId => {
  if (companyId == null || companyId === '') {
    return buildDefaultHistoryFilters();
  }

  const storedFilters = resolveStoredTableFiltersPreference(
    buildOrderHistoryFilterPreferenceScope(companyId),
  );
...
export default function OrderHistoryPage({ navigation, route }) {
  ...
  const [historyFilters, setHistoryFilters] = useState(() =>
    resolveInitialHistoryFilters(currentCompany?.id),
  );
  ...
  const applyHistoryFilters = useCallback(
    nextFilters => {
      ...
      persistTableFiltersPreference(
        buildOrderHistoryFilterPreferenceScope(currentCompany?.id),
        resolvedFilters,
      );
  ...
  }, [currentCompany?.id, orderActions, statusOptions]);
