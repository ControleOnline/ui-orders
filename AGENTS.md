...
- `OrderHistoryPage` React pertence a `ui-orders`, mesmo quando acessada pelo `MANAGER`. A listagem deve usar `DefaultTable` React com busca via `searchProps` na toolbar, com `OrderHeader` apenas como renderer de card compacto/customizado.
